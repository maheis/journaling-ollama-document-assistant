# joda - journaling ollama document assistant
...
```bash
cd ~/.local/share/joda
systemctl --user stop joda.service
git pull --ff-only
bash ./install.sh --install-dir ~/.local/share/joda
systemctl --user start joda.service
systemctl --user status joda.service
```
...
```bash
git clone https://github.com/maheis/journaling-ollama-document-assistant.git /tmp/joda-update
bash /tmp/joda-update/install.sh --install-dir ~/.local/share/joda
...
- Datei: `systemd/joda.service`

Installation:

```bash
mkdir -p ~/.config/systemd/user
cp ./systemd/joda.service ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now joda.service
systemctl --user status joda.service
```
...
```bash
bash ~/.local/share/joda/uninstall.sh
```
...
```bash
bash ~/.local/share/joda/uninstall.sh --remove-ollama --remove-models
```
...
systemctl --user stop joda.service
systemctl --user disable joda.service
rm -f ~/.config/systemd/user/joda.service
systemctl --user daemon-reload
systemctl --user reset-failed

# 2) Installationsverzeichnis entfernen
rm -rf ~/.local/share/joda
